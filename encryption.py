...
def feistel_function(right, subkey):
    ...
    xored = [r ^ k for r, k in zip(expanded_right, subkey)]
    output = []
    for i in range(8):
        ...
        bits = [int(b) for b in bin(value)[2:].rjust(4, '0')]
        output.extend(bits)
    permuted_output = permute(output, P)
    return permuted_output

def des_encrypt(block, key):
    block = initial_permutation(block, IP)
    left = block[:32]
    right = block[32:]
    subkeys = key_schedule(key)
    for subkey in subkeys:
        temp_right = right.copy()
        right = [l ^ r for l, r in zip(left, feistel_function(right, subkey))]
        left = temp_right
    combined = right + left
    cipher_block = final_permutation(combined, FP)
    return cipher_block
...
def des_decrypt(block, key):
    ...
    for subkey in reversed(subkeys):
        ...
        left = temp_right
    combined = right + left
    plain_block = final_permutation(combined, FP)
    return plain_block
...
def encrypt_text(plaintext, key_text):
    ...
    return bit_array_to_hex(cipher_bits)
...
def decrypt_text(ciphertext, key_text):
    ...
    for i in range(0, len(ciphertext), 2):
        ...
        bit_array.extend([int(x) for x in bin(int(hex_byte, 16))[2:].rjust(8, '0')])
    plain_bits = []
    for i in range(0, len(bit_array), 64):
        block = bit_array[i:i+64]
        decrypted_block = des_decrypt(block, key)
        plain_bits.extend(decrypted_block)
    return bit_array_to_text(plain_bits).strip()

def main(action, given_text, key_text):
    if action == 'e':
        cipher_text = encrypt_text(given_text, key_text)
        return cipher_text
    elif action == 'd':
        decrypted_text = decrypt_text(given_text, key_text)
