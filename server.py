...
server.bind((host, port))  # Binding server to the defined host and port
server.listen()
server.settimeout(1)
...
def handle(client):
  while True:
    try:
      ...
      print(message)  # This will print correctly
  ...
  try:
    while True:
      try:
        ...
        # Use one thread for this particular client
        thread = threading.Thread(target=handle, args=(client,))
        thread.daemon = True
        thread.start()

      # Timeout to allow loop to check for keyboard interrupt periodically
